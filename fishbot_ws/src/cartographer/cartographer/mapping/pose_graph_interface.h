...
class PoseGraphInterface {
 public:
  ...
  // Updates the global pose of a trajectory node. This is used for
  // relocalization during pure localization mode.
  // For pure localization, the trajectory has no nodes, so we store
  // the pose directly in initial_trajectory_poses.
  virtual void UpdateTrajectoryPose(int trajectory_id,
