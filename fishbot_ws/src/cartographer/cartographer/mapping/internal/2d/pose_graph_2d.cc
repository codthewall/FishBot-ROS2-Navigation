...
void PoseGraph2D::UpdateTrajectoryPose(const int trajectory_id,
                                       const transform::Rigid3d& pose) {
  absl::MutexLock locker(&mutex_);
  // For pure localization mode, we update the initial trajectory pose
  // to point to itself with the new pose. This allows the local-to-global
  // transform to be updated.
  data_.initial_trajectory_poses[trajectory_id] =
      InitialTrajectoryPose{trajectory_id, pose, common::Time::min()};
}
