    ...
    : node_options_(node_options)
{
  ...
  relocalize_server_ = node_->create_service<cartographer_ros_msgs::srv::Relocalize>(
      "/relocalize",
      std::bind(
          &Node::handleRelocalize, this, std::placeholders::_1, std::placeholders::_2));
...
bool Node::handleRelocalize(
    const cartographer_ros_msgs::srv::Relocalize::Request::SharedPtr request,
    cartographer_ros_msgs::srv::Relocalize::Response::SharedPtr response) {
  absl::MutexLock lock(&mutex_);
  ...
  response->status = TrajectoryStateToStatus(
      ...
      {TrajectoryState::ACTIVE, TrajectoryState::FINISHED,
       TrajectoryState::FROZEN} /* valid states */);
  if (response->status.code != cartographer_ros_msgs::msg::StatusCode::OK) {
    LOG(ERROR) << "Can't relocalize trajectory: " << response->status.message;
    return true;
  }

  // Convert the pose from geometry_msgs to Rigid3d
  const auto pose = ToRigid3d(request->pose);
  if (!pose.IsValid()) {
    response->status.message =
        "Invalid pose argument. Orientation quaternion must be normalized.";
    LOG(ERROR) << response->status.message;
  ...
  // Update the trajectory pose
  map_builder_bridge_->UpdateTrajectoryPose(request->trajectory_id, pose);
